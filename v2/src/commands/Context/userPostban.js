        ...
        bot.connection.query(`SELECT * FROM users WHERE userId = '${interaction.targetId}'`, function(err, userResult) {
            if(userResult[0]) {
                let embed = new MessageEmbed()
                    ...
                    .setTimestamp()
                    if(userResult[0].postban) {
                        embed.setDescription(`[${userResult[0].userName}](${bot.baseOpts.url}/user/${userResult[0].userId}) is banned from posting bot to the website.`)
                    } else {
                        embed.setDescription(`[${userResult[0].userName}](${bot.baseOpts.url}/user/${userResult[0].userId}) is not postban.`)
                    }
